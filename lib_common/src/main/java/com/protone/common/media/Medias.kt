...
import com.protone.worker.DatabaseBridge
...
suspend fun photoInToday(): GalleryMedia? =
    getOldDateMedia(com.protone.worker.DatabaseBridge.instance.galleryDAOBridge.getAllMediaByType(false))

suspend fun videoInToday(): GalleryMedia? =
    getOldDateMedia(com.protone.worker.DatabaseBridge.instance.galleryDAOBridge.getAllMediaByType(true))
...
fun randomNote(): Note? {
    val allNote = runBlocking(Dispatchers.IO) {
        com.protone.worker.DatabaseBridge.instance.noteDAOBridge.getAllNote()
    }
